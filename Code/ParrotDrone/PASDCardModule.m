...
- (void)getAllFlightMedias {
    if (!_isCurrentlyDownloading) {
        _isCurrentlyDownloading = YES;
        [self performSelectorInBackground:@selector(getAllFlightMediasInBg) withObject:nil];
    }
}
...
- (void)getFlightMediasInBg:(NSString*)runId {
    NSArray *mediasList = [self getMediasListWithThumb:false];
...
- (NSArray*)getMediasListWithThumb:(BOOL)thumb {
    ...
    mediasListCount = ARDATATRANSFER_MediasDownloader_GetAvailableMediasSync(_manager,thumb,&result); // thumb
    if (result == ARDATATRANSFER_OK) {
...
void medias_downloader_progress_callback(void* arg, ARDATATRANSFER_Media_t *media, float percent) {
    PASDCardModule *sdCardModule = (__bridge PASDCardModule*)arg;
    int progressAsInt = floor(percent);
    if (sdCardModule.lastProgressSent != progressAsInt) {
        sdCardModule.lastProgressSent = progressAsInt;
        PaMediaModel *mediaModel = [[PaMediaModel alloc] initWithMedia:media];
        [sdCardModule.delegate sdcardModule:sdCardModule media:mediaModel downloadDidProgress:percent];
    }
}
...
void medias_downloader_completion_callback(void* arg, ARDATATRANSFER_Media_t *media, eARDATATRANSFER_ERROR error) {
    PASDCardModule *sdCardModule = (__bridge PASDCardModule*)arg;
    PaMediaModel *mediaModel = [[PaMediaModel alloc] initWithMedia:media];
    [sdCardModule.delegate sdcardModule:sdCardModule mediaDownloadDidFinish:mediaModel];
